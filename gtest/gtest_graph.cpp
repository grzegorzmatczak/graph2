...
namespace gtest_graph {
    ...
    TEST_F(GTest_graph, test_testGraph_1)
    {
		...
		const QJsonObject _graphConfig = cf.readObjectConfig(path+TEST_GRAPH);
		const QJsonObject _blockConfiguration = cf.readObjectConfig(path+TEST_GRAPH_CONFIG);
		const QJsonArray graphConfig = _graphConfig[TEST_GRAPH_NAME].toArray();
		const QJsonArray blockConfiguration = _blockConfiguration[TEST_GRAPH_NAME].toArray();
